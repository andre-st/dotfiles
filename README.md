...
flowchart TD
	...
	sh("mybackup.sh")
	rsync("notify-send --expire-time=0 'Backup running' <br> && systemctl start systemd-cryptsetup@... <br> && mount /mnt/backup <br> && rsync --backup-dir=/mnt/backup/changed/$NOW /mnt/data /mnt/backup/latest <br> && umount ... && systemctl stop ... <br> && notify-send --expire-time=0 'Success' <br>|| notify-send --urgency=critical 'Error'")
